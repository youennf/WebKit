...
#import <webrtc/common_video/h265/h265_bitstream_parser.h>
...
#import <webrtc/system_wrappers/include/clock.h>
...
static bool CopyVideoFrameToPixelBuffer(id<RTCI420Buffer> frameBuffer, CVPixelBufferRef pixelBuffer)
{
...
@implementation RTCVideoEncoderH265 {
    ...
    RetainPtr<CFStringRef> _profile;
    RTCVideoEncoderCallback _callback;
    int32_t _width;
    int32_t _height;
    RetainPtr<VTCompressionSessionRef> _compressionSession;
           ...
           frameTypes:(NSArray<NSNumber *> *)frameTypes
{
    ...
    CVPixelBufferPoolRef pixelBufferPool = VTCompressionSessionGetPixelBufferPool(_compressionSession.get());
    ...
    encodeParams.reset(new RTCFrameEncodeParams(self, _width, _height, frame.timeStampNs / rtc::kNumNanosecsPerMillisec, frame.timeStamp, frame.rotation));

    // Update the bitrate if needed.
    [self setBitrateBps:_bitrateAdjuster->GetAdjustedBitrateBps()];

    OSStatus status = VTCompressionSessionEncodeFrame( _compressionSession.get(), pixelBuffer, presentationTimeStamp, kCMTimeInvalid, frameProperties, encodeParams.release(), nullptr);
    if (frameProperties)
...
- (int)resetCompressionSession {
...
#endif
    VTCompressionSessionRef compressionSessionOut = nullptr;
    OSStatus status = VTCompressionSessionCreate(
        ...
        compressionOutputCallback, nullptr, &compressionSessionOut);
    if (status != noErr) {
        if (encoder_specs)
    ...
    _compressionSession = adoptCF(compressionSessionOut);
#if defined(WEBRTC_MAC) && !defined(WEBRTC_IOS)
    CFBooleanRef hwaccl_enabled = nullptr;
    status = VTSessionCopyProperty(_compressionSession.get(), kVTCompressionPropertyKey_UsingHardwareAcceleratedVideoEncoder, nullptr, &hwaccl_enabled);
...
- (void)configureCompressionSession {
    ...
    SetVTSessionProperty(_compressionSession.get(), kVTCompressionPropertyKey_RealTime, _isLowLatencyEnabled);
    SetVTSessionProperty(_compressionSession.get(), kVTCompressionPropertyKey_AllowFrameReordering, false);
    [self setEncoderBitrateBps:_targetBitrateBps];

    // Set a relatively large value for keyframe emission (7200 frames or 4 minutes).
    SetVTSessionProperty(_compressionSession.get(), kVTCompressionPropertyKey_MaxKeyFrameInterval, 7200);
    SetVTSessionProperty(_compressionSession.get(), kVTCompressionPropertyKey_MaxKeyFrameIntervalDuration, 240);
    OSStatus status = VTCompressionSessionPrepareToEncodeFrames(_compressionSession.get());
    if (status != noErr)
...
- (void)destroyCompressionSession {
    if (_compressionSession) {
        VTCompressionSessionInvalidate(_compressionSession.get());
...
- (void)setEncoderBitrateBps:(uint32_t)bitrateBps {
    if (_compressionSession) {
        SetVTSessionProperty(_compressionSession.get(), kVTCompressionPropertyKey_AverageBitRate, bitrateBps);
...
- (void)frameWasEncoded:(OSStatus)status
    ...
    if (_useAnnexB) {
        ...
        while (currentStart < size) {
            ...
            if (auto error = CMBlockBufferGetDataPointer(blockBuffer, currentStart, &length, nullptr, &data)) {
                RTC_LOG(LS_ERROR) << "H265 decoder: CMBlockBufferGetDataPointer failed with error " << error;
                if (_errorCallback)
                    _errorCallback(ErrorCallbackDefaultValue);
                return;
            }
    ...
    frame.contentType = (_mode == RTCVideoCodecModeScreensharing) ? RTCVideoContentTypeScreenshare : RTCVideoContentTypeUnspecified;
...
- (RTCVideoEncoderQpThresholds*)scalingSettings {
    return [[RTCVideoEncoderQpThresholds alloc] initWithThresholdsLow:kLowh265QpThreshold high:kHighh265QpThreshold];
}

- (void)flush {
    if (_compressionSession)
        VTCompressionSessionCompleteFrames(_compressionSession.get(), kCMTimeInvalid);
}
