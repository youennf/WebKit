...
#import "VideoProcessingSoftLink.h"
#import "WebKitUtilities.h"
#import "nalu_rewriter.h"
#import "helpers.h"

#import <libyuv/convert_from.h>
#import <dlfcn.h>
#import <objc/runtime.h>
ALLOW_UNUSED_PARAMETERS_BEGIN
#import <webrtc/api/video_codecs/h264_profile_level_id.h>
#import <webrtc/common_video/h264/h264_bitstream_parser.h>
#import <webrtc/common_video/include/bitrate_adjuster.h>
#import <webrtc/modules/include/module_common_types.h>
#import <webrtc/modules/video_coding/include/video_error_codes.h>
#import <webrtc/rtc_base/buffer.h>
#import <webrtc/rtc_base/logging.h>
#import <webrtc/rtc_base/system/arch.h>
...
struct RTCFrameEncodeParams {
    RTCFrameEncodeParams(RTCVideoEncoderH264 *e,
        ...
        , duration(duration)
        , rotation(r)
        , isKeyFrameRequired(isKeyFrameRequired)
    {
        if (csi)
...
static CVPixelBufferRef CreatePixelBuffer(VTCompressionSessionRef compression_session) {
                               ...
                               CMSampleBufferRef sampleBuffer)
{
    ...
    std::unique_ptr<RTCFrameEncodeParams> encodeParams(reinterpret_cast<RTCFrameEncodeParams *>(params));
...
static CFStringRef ExtractProfile(const webrtc::H264ProfileLevelId &profile_level_id)
{
...
// for details.
static NSUInteger GetMaxSampleRate(const webrtc::H264ProfileLevelId &profile_level_id)
{
    switch (profile_level_id.level) {
    case webrtc::H264Level::kLevel3:
        return 10368000;
...
static size_t GetMaxBitRateKBps(const webrtc::H264ProfileLevelId &profile_level_id)
{
    switch (profile_level_id.level) {
    case webrtc::H264Level::kLevel1:
        return 64;
...
static size_t computeDefaultBitRateKbps(uint32_t proposedBitrateKbps, const webrtc::H264ProfileLevelId& profile_level_id)
{
    return proposedBitrateKbps ? proposedBitrateKbps : GetMaxBitRateKBps(profile_level_id);
}

static uint32_t computeFramerate(uint32_t proposedFramerate, uint32_t maxAllowedFramerate)
{
...
@implementation RTCVideoEncoderH264 {
    RetainPtr<RTCVideoCodecInfo> _codecInfo;
    ...
    bool _useVCP;
    bool _useBaseline;
    RetainPtr<VTCompressionSessionRef> _vtCompressionSession;
...
- (instancetype)initWithCodecInfo:(RTCVideoCodecInfo *)codecInfo {
    if (self = [super init]) {
        ...
        _profile_level_id = webrtc::ParseSdpForH264ProfileLevelId([codecInfo nativeSdpVideoFormat].parameters);
...
- (NSInteger)startEncodeWithSettings:(RTCVideoEncoderSettings *)settings
    ...
    _maxAllowedFrameRate = static_cast<uint32_t>(GetMaxSampleRate(*_profile_level_id) / (aligned_width * aligned_height));
...
- (NSInteger)encode:(RTCVideoFrame *)frame
    ...
    if ([frame.buffer isKindOfClass:[RTCCVPixelBuffer class]]) {
        ...
        if (![rtcPixelBuffer requiresCropping]) {
            ...
            pixelBuffer = CreatePixelBuffer(_vtCompressionSession.get());
            if (!pixelBuffer) {
                if (_errorCallback)
                    _errorCallback(ErrorCallbackDefaultValue);
                return WEBRTC_VIDEO_CODEC_ERROR;
            ...
            if ([rtcPixelBuffer requiresScalingToWidth:dstWidth height:dstHeight]) {
                int size = [rtcPixelBuffer bufferSizeForCroppingAndScalingToWidth:dstWidth height:dstHeight];
    ...
    if (_vtCompressionSession) {
        status = PAL::VTCompressionSessionEncodeFrame(_vtCompressionSession.get(),
                                                    pixelBuffer,
...
- (BOOL)resetCompressionSessionIfNeededWithFrame:(RTCVideoFrame *)frame {
    ...
    if ([self hasCompressionSession]) {
        CVPixelBufferPoolRef pixelBufferPool = VTCompressionSessionGetPixelBufferPool(_vtCompressionSession.get());
        if (!pixelBufferPool)
            return NO;

        // The pool attribute `kCVPixelBufferPixelFormatTypeKey` can contain either an array of pixel
        // formats or a single pixel format.
        NSDictionary *poolAttributes = (__bridge NSDictionary *)WebCore::CVPixelBufferPoolGetPixelBufferAttributes(pixelBufferPool);
        ...
        if (![compressionSessionPixelFormats containsObject:[NSNumber numberWithLong:framePixelFormat]]) {
...
- (int)resetCompressionSessionWithPixelFormat:(OSType)framePixelFormat {
    ...
    auto useHardwareEncoder = isH264HardwareEncoderAllowed() ? kCFBooleanTrue : kCFBooleanFalse;
    ...
    VTCompressionSessionRef vtCompressionSessionOut = nullptr;
  ...
  // Provided encoder should be good enough.
    ...
    _vtCompressionSession = adoptCF(vtCompressionSessionOut);
...
- (void)configureCompressionSession {
    ...
    SetVTSessionProperty(_vtCompressionSession.get(), kVTCompressionPropertyKey_RealTime, _isH264LowLatencyEncoderEnabled);
#if ENABLE_VCP_FOR_H264_BASELINE
    if (_useBaseline && _useVCP && _isH264LowLatencyEncoderEnabled)
        SetVTSessionProperty(_vtCompressionSession.get(), kVTCompressionPropertyKey_ProfileLevel, kVTProfileLevel_H264_ConstrainedBaseline_AutoLevel);
    else
#endif
        SetVTSessionProperty(_vtCompressionSession.get(), kVTCompressionPropertyKey_ProfileLevel, ExtractProfile(*_profile_level_id));
    SetVTSessionProperty(_vtCompressionSession.get(), kVTCompressionPropertyKey_AllowFrameReordering, false);
    if (_enableL1T2ScalabilityMode) {
        const double kL1T2Fraction = 0.5;
        SetVTSessionProperty(_vtCompressionSession.get(), kVTCompressionPropertyKey_BaseLayerFrameRateFraction, kL1T2Fraction);
    }

    [self setEncoderBitrateBps:_targetBitrateBps frameRate:_encoderFrameRate];
    // Set a relatively large value for keyframe emission (7200 frames or 4 minutes).
    SetVTSessionProperty(_vtCompressionSession.get(), kVTCompressionPropertyKey_MaxKeyFrameInterval, 7200);
    SetVTSessionProperty(_vtCompressionSession.get(), kVTCompressionPropertyKey_MaxKeyFrameIntervalDuration, 240);
}

- (void)destroyCompressionSession {
    if (_vtCompressionSession) {
        VTCompressionSessionInvalidate(_vtCompressionSession.get());
...
- (void)setEncoderBitrateBps:(uint32_t)bitrateBps frameRate:(uint32_t)frameRate {
    if ([self hasCompressionSession]) {
        ...
        if (actualTarget)
            SetVTSessionProperty(_vtCompressionSession.get(), kVTCompressionPropertyKey_AverageBitRate, actualTarget);

        // With zero |_maxAllowedFrameRate|, we fall back to automatic frame rate detection.
        if (_maxAllowedFrameRate > 0)
            SetVTSessionProperty(_vtCompressionSession.get(), kVTCompressionPropertyKey_ExpectedFrameRate, frameRate);
...
- (void)frameWasEncoded:(OSStatus)status
    ...
    frame.buffer = [[NSData alloc] initWithBytesNoCopy:buffer->data()
                                                length:buffer->size()
                                           deallocator:^(void*, NSUInteger) { buffer.reset(); }];
...
- (void)flush {
    if (_vtCompressionSession)
        VTCompressionSessionCompleteFrames(_vtCompressionSession.get(), kCMTimeInvalid);
}
